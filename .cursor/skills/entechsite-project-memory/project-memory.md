...
### 2026-06-25 - Подпапки Drive как отдельные уроки

- Goal: сделать структуру обучения читабельной: курсы в алфавитном порядке, подпапки внутри курса — отдельные уроки с материалами внутри.
- Changes:
  - Drive sync сортирует курсы и элементы курса через `localeCompare('ru', { numeric: true })`;
  - рекурсивное разворачивание всех файлов в плоский список заменено на прямые элементы курса: подпапка = урок, прямой файл = урок;
  - урок-папка сохраняет ссылку на папку Google Drive и описание `Папка материалов Google Drive`;
  - старые Drive-уроки, которых больше нет в текущей структуре курса, переводятся в `is_active = false`;
  - frontend показывает количество отключённых старых уроков после синхронизации.
- Files:
...
- Result: после повторной синхронизации список уроков должен стать сгруппированным по подпапкам и отсортированным, без старой мешанины файлов.
...
### 2026-06-25 - Уроки Drive строятся из прямых подпапок курса

- Context: рекурсивная раскладка всех файлов из вложенных папок создавала перемешанный список уроков (`pdf`, `mp4`, `txt`) вместо логичных блоков.
- Decision: считать курсом папку первого уровня в «Обучение», а уроком — каждый прямой элемент курса: подпапку или файл. Вложенные файлы остаются внутри папки-урока и открываются через ссылку на Google Drive.
- Trade-off: отдельные файлы внутри подпапки больше не становятся самостоятельными уроками; зато пользователь видит один урок с набором материалов внутри.
