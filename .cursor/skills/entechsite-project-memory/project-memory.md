...
- Changes:
  ...
  - файлы внутри курсов/уроков и вложенность глубже уровня урока не синхронизируются;
  - после sync активными остаются только уроки-прямые подпапки курса, старые файловые/лишние уроки скрываются через `is_active=false`;
...
- Result: можно автоматически наполнить базу знаний из Drive папками-курсами и папками-уроками; материалы внутри папок остаются только в Google Drive.
